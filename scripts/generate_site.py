"""Generate the static Guoxue content and compliance dashboard."""
...
POLICIES_FILE = ROOT / "content" / "policies.json"
...
/* Compliance radar */
.badge.topic-易学民俗,.badge.topic-中医文化{background:rgba(163,58,43,.09);color:var(--red)}.badge.topic-数字国学{background:rgba(184,139,68,.14);color:#886018}.risk-grid{display:grid;grid-template-columns:repeat(3,1fr);gap:12px}.risk-card{padding:21px;border:1px solid var(--line);border-radius:15px;background:var(--card)}.risk-card.low{border-top:4px solid #258665}.risk-card.mid{border-top:4px solid #b88b44}.risk-card.high{border-top:4px solid #b84a3b}.risk-card h3{margin-bottom:8px}.policy-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:14px}.policy-card{padding:22px;border:1px solid var(--line);border-radius:16px;background:var(--card);box-shadow:var(--shadow)}.policy-card h3{margin:11px 0 6px}.policy-meta{display:flex;justify-content:space-between;gap:12px;align-items:center}.policy-tags{display:flex;flex-wrap:wrap;gap:6px;margin:13px 0}.change{padding:4px 9px;border-radius:999px;font-size:11px;font-weight:900;background:#ece9df;color:var(--muted)}.change.changed{background:#fff0d2;color:#7b4f00}.change.unreachable{background:#f7dfdc;color:#8f2d22}.checker{display:grid;grid-template-columns:180px 1fr auto;gap:10px;margin-top:16px}.checker select,.checker textarea{border:1px solid var(--line);border-radius:10px;background:#fff;padding:12px;font:inherit}.checker textarea{min-height:92px;resize:vertical}.checker button{border:0;border-radius:10px;background:var(--jade);color:#fff;padding:0 20px;font-weight:900;cursor:pointer}.check-result{margin-top:12px;padding:14px;border-radius:10px;background:var(--paper2);font-size:13px}.check-result.warn{background:#fff0d2;color:#714a08}.check-result.high{background:#f7dfdc;color:#7e291f}
@media(max-width:980px){.feed{grid-template-columns:repeat(2,1fr)}.source-grid{grid-template-columns:repeat(3,1fr)}.toolbar{grid-template-columns:1fr}.toolbar{position:static}.policy-grid{grid-template-columns:1fr}}
@media(max-width:680px){.topbar-inner,main{width:min(100% - 24px,1240px)}.topbar-inner{align-items:flex-start;flex-direction:column;padding:13px 0}nav{width:100%;justify-content:flex-start;flex-wrap:wrap;gap:8px 16px}.hero{padding:38px 24px;border-radius:18px}.hero:after{font-size:160px}.stats{grid-template-columns:repeat(2,1fr)}.feed,.source-grid,.risk-grid{grid-template-columns:1fr}.checker{grid-template-columns:1fr}.checker button{padding:12px}.section-head{align-items:flex-start;flex-direction:column}.topic-row,.archive-row{grid-template-columns:1fr;gap:5px}}
"""
...
def nav() -> str:
    return '<a href="./">今日内容</a><a href="compliance.html">合规雷达</a><a href="sources.html">平台矩阵</a><a href="topics.html">选题库</a><a href="archive.html">归档</a>'


def shell(title: str, body: str) -> str:
    return f"""<!doctype html><html lang="zh-CN"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta name="description" content="追踪国学内容趋势与平台合规规则"><title>{esc(title)}</title><style>{CSS}</style></head><body><header class="topbar"><div class="topbar-inner"><a class="brand" href="./"><span class="seal">国学<br>合规</span><span>国学情报站</span></a><nav>{nav()}</nav></div></header><main>{body}</main></body></html>"""


def item_card(item: dict) -> str:
    topic = item.get("topic") or "经典文化"
...
def render_index(config: dict, report: dict, now: datetime) -> str:
    ...
    topics = config.get("topics") or ["经典文化", "非遗民俗", "易学民俗", "中医文化", "数字国学"]
    ...
    sensitive = counts["易学民俗"] + counts["中医文化"]
    digital = counts["数字国学"]
    body = f"""<section class="hero"><p class="eyebrow">Guoxue Content &amp; Compliance Radar</p><h1>{esc(config.get('site_name'))}</h1><p class="lead">{esc(config.get('tagline'))}</p><span class="pulse"><i></i>最近更新：{esc(generated.replace('T',' '))}</span></section><section class="stats"><div class="stat"><strong>{len(items)}</strong><span>本期公开信息</span></div><div class="stat"><strong>{len(platforms)}</strong><span>已捕获平台</span></div><div class="stat"><strong>{sensitive}</strong><span>合规敏感选题</span></div><div class="stat"><strong>{digital}</strong><span>数字国学信号</span></div></section><section class="toolbar"><input id="q" class="search" type="search" placeholder="搜索人物、经典、民俗、平台或标题…" aria-label="搜索情报"><div class="filters"><button class="filter active" data-filter="topic" data-value="全部">全部主题</button>{topic_buttons}</div><div class="filters"><button class="filter active" data-filter="platform" data-value="全部">全部平台</button>{platform_buttons}</div></section><section class="section"><div class="section-head"><h2>今日国学内容流</h2><p>当前显示 <span id="visible" class="hidden-count">{len(items)}</span> 条 · 点击原文回到信息源核验</p></div><div class="feed">{cards}<div class="empty" id="no-results" style="display:none">没有匹配结果，换个关键词或筛选条件试试。</div></div></section><footer>信息来自公开页面与 RSS 检索结果；本站只做索引、摘要和分类，版权归原作者与平台所有。生成时间：{esc(now.strftime('%Y-%m-%d %H:%M'))} Asia/Shanghai。</footer>{FILTER_JS}"""
    return shell(config.get("site_name", "国学内容与合规情报站"), body)
...
def render_sources(config: dict, report: dict, now: datetime) -> str:
    ...
    body = f"""<section class="hero"><p class="eyebrow">Source Matrix</p><h1>国学内容平台矩阵</h1><p class="lead">覆盖中文内容生态与全球社区中的国学、非遗、经典文化和数字国学信号。绿点表示最近一次采集成功，红点表示该来源临时不可达。</p></section><section class="section"><div class="section-head"><h2>{len(cards)} 个公开来源</h2><p>站点不登录账号，不采集私域内容</p></div><div class="source-grid">{''.join(cards)}</div></section><section class="section panel"><h2>采集边界</h2><p class="muted" style="margin-top:10px">通过公开搜索 RSS 建立索引；不绕过登录、验证码、反爬或付费墙。平台搜索结果会受搜索引擎收录影响，因此“0 条”不等于平台当天没有相关内容。每条信息都保留原始链接，建议发布或引用前回源核验。</p></section><footer>页面生成：{esc(now.strftime('%Y-%m-%d %H:%M'))} Asia/Shanghai。</footer>"""
    return shell("平台矩阵 · 国学情报站", body)


def render_compliance(config: dict, policy_report: dict, now: datetime) -> str:
    policies = policy_report.get("policies") or []
    accessible = sum(1 for item in policies if item.get("available"))
    changed = sum(1 for item in policies if item.get("change_status") == "changed")
    checked_at = policy_report.get("generated_at") or now.isoformat(timespec="minutes")
    status_labels = {"baseline": "已建立基线", "unchanged": "未发现变化", "changed": "疑似有变化", "unreachable": "暂不可达"}
    cards = []
    for item in policies:
        status = item.get("change_status", "baseline")
        focus = "".join(f'<span class="badge">{esc(tag)}</span>' for tag in item.get("focus", []))
        matches = []
        for match in (item.get("matches") or [])[:3]:
            matches.append(esc(match.get("category")))
        hit_text = "、".join(matches) or "等待正文识别"
        cards.append(f"""<article class="policy-card"><div class="policy-meta"><strong>{esc(item.get('platform'))}</strong><span class="change {esc(status)}">{esc(status_labels.get(status, status))}</span></div><h3>{esc(item.get('title'))}</h3><p class="muted">本页重点：{hit_text}</p><div class="policy-tags">{focus}</div><p>{esc(item.get('guidance'))}</p><div class="card-foot"><span class="source">核验：{esc(item.get('checked_at','').replace('T',' '))}</span><a class="open" href="{safe_url(item.get('url',''))}" target="_blank" rel="noopener noreferrer">官方规则 ↗</a></div></article>""")
    cards_html = "".join(cards) or '<div class="empty">暂无政策数据，请先运行规则采集。</div>'
    checker_js = r"""
<script>
const riskGroups={
  "高风险承诺":["改运","转运","招财","消灾","算命","看相","测姻缘","百分百","保证灵验"],
  "医疗健康":["治愈","治疗","疗效","处方","替代就医","包好","根治"],
  "宗教与迷信":["做法事","开光保佑","驱邪","通灵","前世","因果报应"],
  "营销与引流":["私信购买","加微信","付费测算","大师一对一","名额有限"]
};
document.querySelector('#check-btn')?.addEventListener('click',()=>{
  const text=document.querySelector('#check-text').value.trim();
  const platform=document.querySelector('#check-platform').value;
  const hits=[];
  Object.entries(riskGroups).forEach(([group,words])=>{const found=words.filter(word=>text.includes(word));if(found.length)hits.push(`${group}：${found.join('、')}`)});
  const result=document.querySelector('#check-result');
  result.className='check-result'+(hits.some(x=>x.startsWith('高风险')||x.startsWith('医疗'))?' high':hits.length?' warn':'');
  result.innerHTML=hits.length?`<strong>${platform} 发布前建议复核</strong><br>${hits.join('<br>')}<br><small>建议改为有出处的文化介绍，删除确定性承诺，并回到平台官方规则核验。</small>`:`<strong>初筛未命中预设高风险词</strong><br><small>这不代表内容一定合规；仍需核对事实、版权、资质、商业推广和平台最新规则。</small>`;
});
</script>
"""
    platform_options = "".join(f'<option>{esc(item.get("platform"))}</option>' for item in policies)
    body = f"""<section class="hero"><p class="eyebrow">Platform Compliance Radar</p><h1>国学内容合规雷达</h1><p class="lead">集中查看平台官方规则、变化状态与国学内容发布风险。这里只做运营初筛，不代替平台最终审核或法律意见。</p><span class="pulse"><i></i>规则核验：{esc(checked_at.replace('T',' '))}</span></section><section class="stats"><div class="stat"><strong>{len(policies)}</strong><span>监测平台</span></div><div class="stat"><strong>{accessible}</strong><span>本次可访问</span></div><div class="stat"><strong>{changed}</strong><span>疑似规则变化</span></div><div class="stat"><strong>{sum(len(i.get('focus') or []) for i in policies)}</strong><span>风险关注点</span></div></section><section class="section"><div class="section-head"><h2>国学内容风险分层</h2><p>判断表达方式，不简单给传统文化题材贴标签</p></div><div class="risk-grid"><article class="risk-card low"><h3>低风险 · 资料型</h3><p class="muted">经典原文、非遗技艺、历史资料、博物馆与学术研究。保留出处，避免断章取义。</p></article><article class="risk-card mid"><h3>需复核 · 民俗解释</h3><p class="muted">节气、生肖、易学、养生等内容要明确是文化介绍或个人观点，不作现实结果承诺。</p></article><article class="risk-card high"><h3>高风险 · 功效承诺</h3><p class="muted">算命改运、招财消灾、疾病治疗、恐吓营销、付费测算与私域引流，发布前重点审查。</p></article></div></section><section class="section panel"><div class="section-head"><h2>发布前快速初筛</h2><p>关键词规则仅作提示</p></div><div class="checker"><select id="check-platform" aria-label="目标平台">{platform_options}</select><textarea id="check-text" placeholder="粘贴标题、口播稿或文案，例如：大师一对一帮你改运招财……"></textarea><button id="check-btn">开始初筛</button></div><div id="check-result" class="check-result">输入文案后点击“开始初筛”。</div></section><section class="section"><div class="section-head"><h2>官方规则监测</h2><p>“疑似有变化”表示页面指纹变化，需打开官方规则人工确认</p></div><div class="policy-grid">{cards_html}</div></section><footer>{esc(policy_report.get('disclaimer') or '仅提供平台运营风险提示，不构成法律意见；发布前请回到平台官方规则核验。')}</footer>{checker_js}"""
    return shell("合规雷达 · 国学情报站", body)
...
def main() -> None:
    ...
    policies = load_json(POLICIES_FILE, {"policies": []})
    ...
    (PUBLIC / "compliance.html").write_text(render_compliance(config, policies, now), encoding="utf-8")
    ...
    (data_dir / "policies.json").write_text(json.dumps(policies, ensure_ascii=False, indent=2), encoding="utf-8")
